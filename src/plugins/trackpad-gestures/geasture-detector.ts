...
export class GestureDetector {
    ...
    gestureData: EventData | null = null
    ...
    handleEventDetected() {
        this.events.dispatchEvent(new CustomEvent<EventData>('gesture-detected', { detail: this.gestureData ?? undefined }))
    }

    handleEventProgress() {
        this.events.dispatchEvent(new CustomEvent<EventData>('gesture-in-progress', { detail: this.gestureData ?? undefined }))
    }

    handleEventEnded(shouldHandle: boolean) {
        this.waitForAnotherStart = true
        this.gestureData!.handle = shouldHandle
        const event = new CustomEvent<EventData>('gesture-ended', { detail: this.gestureData ?? undefined })
    ...
    detectEventType() {
        const absVertical = Math.abs(this.gestureData?.sumVertical ?? 0)
        const absHorizontal = Math.abs(this.gestureData?.sumHorizontal ?? 0)
    ...
    handleWheelEvent({
        phase: _phase = Phase.kPhaseChanged,
        momentumPhase: _momentumPhase = Phase.kPhaseNone,
        deltaX,
        deltaY,
        ctrlKey,
