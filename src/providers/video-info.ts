...
export interface VideoInfo {
  ...
  videoId: string;
  playlistId?: string;
  // Compatibility fields for plugins
  artist: string;
  album?: string | null;
  tags?: string[];
  songDuration: number;
...
const handleData = async (
  ...
  if (microformat) {
    ...
    // @ts-expect-error: tags might not exist in definition but exists in runtime or needs update
    videoInfo.tags = Array.isArray(microformat.tags) ? microformat.tags : [];
  }
  ...
  if (videoDetails) {
    ...
    videoInfo.album = videoDetails.album;
    videoInfo.artist = videoInfo.author;
    videoInfo.songDuration = videoInfo.videoDuration;
