...
export const getVideoControls = (win: BrowserWindow) => {
  return {
    // Playback
    ...
    like: () => win.webContents.send('ytd:update-like', LikeType.Like),
    dislike: () => win.webContents.send('ytd:update-like', LikeType.Dislike),
    ...
    search: (query: string, params?: string, continuation?: string) =>
      new Promise((resolve) => {
        ...
        win.webContents.send('ytd:search', query, params, continuation);
      }),
  };
};
